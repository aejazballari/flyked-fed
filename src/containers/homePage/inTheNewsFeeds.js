...
import NewsWhite from '../../assets/navIcons/newsWhite.svg';
import * as feedAction from '../../actions/feedAction';
import * as postAction from '../../actions/postAction';

const InTheNewsFeeds = () => {
  const dispatch = useDispatch();
  const newsList = useSelector((state) => state?.feeds.inNews?.results);
  const newsElement = useSelector((state) => state?.post?.scroll.news);
  const [newsloader, setNewsLoader] = useState(true);

  useEffect(() => {
    dispatch(feedAction.getInNews('1', () => setNewsLoader(false)));
  }, []);
  ...
  if (newsloader || !newsList || newsList?.length === 0) {
    return null;
  }

  return (
    <HorizontalFeedsComponent
      elementId={newsElement}
      title="In the news"
      image={NewsWhite}
      feedLoader={newsloader}
      feedData={newsList}
      feedKey="inNews"
      noDataMessage="There are no news for today"
    />
  );
};

export default InTheNewsFeeds;
